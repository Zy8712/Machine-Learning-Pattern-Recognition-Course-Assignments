...
weightVector = zeros(runAlgoNum, colNum+1); % declare a 1 x colNum+1 matrix consisting of zeros

% While loop. Runs the perception algorithm for the amount of times
% specified
while i <= runAlgoNum % if runAlgoNum is equal to 20, it'll run 20 times
    % Establish our matrix of values which contains both the input and
    % corresponding output values. Matrix will have the same number of rows
    % as the input (and output) and the number of columns will be equal to
    % the input plus the output.
    matrixM(:,1:colNum) = dataset_input; % Set the first two columns equal to values from the input matrix.
    matrixM(:,end) = dataset_output; % Set the last column equal to values from the output matrix.
    ...
    % outputs separately.
    dataset_input_mod = matrixM(:,1:colNum); % Matrix for input data points.
    dataset_output_mod = matrixM(:,end); % Matrix for corresponding outputs.
    ...
    weightVector(i,:) = perceptronAlgo(matrixM, dataset_output_mod); %assignment statement    
