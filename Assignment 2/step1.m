...
dataset_input(:,3) = dataset_output; % set input points equal to output results
positive_points = dataset_input(dataset_input(:,3) == 1,:); % extract positive points
negative_points = dataset_input(dataset_input(:,3) == -1,:); % extract negative points

figure; % create a new figure window with default property values

% Plot Positive & Negative Data Points
plot(positive_points(:,1),positive_points(:,2),'+'); % plot positive data points
hold on; % retains plots in the current axes so new plots do not delete it
plot(negative_points(:,1),negative_points(:,2),'o'); % plot negative data points
hold on; % retains plots in the current axes so new plots do not delete it
